...
export function PokemonConnectionsConfigPanel({ config, disabled, onChange }: { config: unknown; disabled: boolean; onChange(config: unknown): Promise<void> }) {
  ...
  return <fieldset disabled={disabled} className="space-y-7">
    ...
    <p className="rounded-xl bg-aqua/[.06] p-3 text-sm font-bold text-ink/70">Cada persona recibe su propia resolución privada del mismo tablero. Los grupos y errores no se revelan al resto hasta que acaba la ronda.</p>
  </fieldset>;
}

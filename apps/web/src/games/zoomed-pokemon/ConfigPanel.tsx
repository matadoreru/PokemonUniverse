import { GENERATIONS, type ZoomedPokemonConfig, type ZoomedPokemonHintKind, type ZoomedPokemonImageMode } from '@pokemon-universe/shared';
...
export function ZoomedPokemonConfigPanel({ config, disabled, onChange }: { config: unknown; disabled: boolean; onChange(config: unknown): Promise<void> }) {
  ...
  const toggleGeneration = (generation: number) => {
    ...
    if (generations.length) update({ generations });
  };

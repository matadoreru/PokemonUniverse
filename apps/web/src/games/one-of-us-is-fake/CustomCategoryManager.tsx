...
export function CustomCategoryManager({
  ...
  description = 'Se guardan en tu cuenta y sirven para futuras salas.',
  placeholder = 'Pokémon que llevarías al gimnasio',
  emptyText = 'Todavía no has creado categorías personales.',
  minimumActive = 2,
  ...
  return <section className="rounded-2xl border border-ink/10 bg-surface-raised/70 p-4" aria-labelledby="custom-categories-title">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><span className="label !mb-0">Cuenta del host</span><h3 id="custom-categories-title" className="font-display text-xl">{title}</h3><p className="mt-1 text-sm font-bold text-ink/60">{description}</p></div>
      <span className={`chip ${enabledCount >= minimumActive ? '!bg-leaf/10 !text-leaf' : '!bg-electric/10 !text-electric'}`}><Check size={14} /> {enabledCount} activas</span>
    </div>
    <div className="mt-4 flex flex-col gap-2 sm:flex-row">
      <label className="min-w-0 flex-1"><span className="sr-only">Nuevo texto personal</span><input className="field min-h-11" maxLength={160} value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} disabled={disabled || busyId === 'new'} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void create(); } }} /></label>
    ...
    {loading ? <p className="py-5 text-sm font-bold text-ink/60">Cargando textos…</p> : categories.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-ink/15 p-4 text-center text-sm font-bold text-ink/55">{emptyText}</p> : <ul className="mt-4 max-h-80 space-y-2 overflow-y-auto pr-1">

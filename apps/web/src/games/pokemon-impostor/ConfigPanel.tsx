...
export function PokemonImpostorConfigPanel({ config, disabled, onChange }: { config: unknown; disabled: boolean; onChange(config: unknown): Promise<void> }) {
  ...
  return <fieldset disabled={disabled}>
    ...
    <div className="mt-6 grid gap-5 md:grid-cols-3">
      ...
      <label><span className="label flex items-center gap-2"><MessageSquareText size={16} /> Pistas · {value.clueSeconds}s</span><input className="w-full accent-aqua" type="range" min={10} max={120} step={5} value={value.clueSeconds} onChange={(event) => void onChange({ ...value, clueSeconds: Number(event.target.value) })} /></label>

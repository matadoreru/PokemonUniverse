import { GENERATIONS, type WhosThatPokemonConfig } from '@pokemon-universe/shared';
...
function ToggleCard({ active, title, description, icon, onClick }: { active: boolean; title: string; description: string; icon: ReactNode; onClick(): void }) {
  return <button type="button" aria-pressed={active} onClick={onClick} className={`flex min-h-20 items-center gap-3 rounded-2xl border-2 p-3 text-left transition ${active ? 'border-aqua bg-aqua/10' : 'border-ink/10 bg-surface-raised text-ink/55'}`}><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${active ? 'bg-aqua text-night' : 'bg-ink/[0.07]'}`}>{icon}</span><span className="min-w-0 flex-1"><strong className="block font-display text-lg text-ink">{title}</strong><small className="block font-bold text-ink/45">{description}</small></span><span className={`grid h-6 w-6 place-items-center rounded-full text-xs font-black ${active ? 'bg-leaf text-night' : 'bg-ink/10'}`}>{active ? '✓' : ''}</span></button>;
}
...
export function WhosThatPokemonConfigPanel({ config, disabled, onChange }: { config: unknown; disabled: boolean; onChange(config: unknown): Promise<void> }) {
  ...
  return <fieldset disabled={disabled} className="space-y-6">
    ...
    <p className="rounded-xl bg-ink/[0.04] p-3 text-sm font-bold text-ink/45">Las formas Gigamax y otras variantes no se activan: el dataset actual solo normaliza de forma fiable las formas regionales.</p>
  </fieldset>;
}

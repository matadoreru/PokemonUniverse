...
export function RoomPage() {
  ...
  if (room.phase === 'LOBBY') return <Lobby room={room} selfId={user.id} onLeave={() => run(context.leaveRoom().then(() => navigate('/play')))} onReady={context.setReady} onStart={context.startGame} onSelectGame={context.selectGame} onConfig={context.updateConfig} onGameConfig={context.updateGameConfig} onSession={context.updateSession} onGameSelection={context.updateGameSelection} onSetRoomRole={context.setRoomRole} onTransferHost={context.transferHost} onKick={context.kick} onEndSession={() => run(context.endSession())} />;

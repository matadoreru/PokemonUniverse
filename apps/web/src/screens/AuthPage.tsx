...
export function AuthPage() {
  ...
  const guestImageInput = useRef<HTMLInputElement>(null);
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const auth = useAuth(); const navigate = useNavigate();
  useEffect(() => () => { if (guestImagePreview) URL.revokeObjectURL(guestImagePreview); }, [guestImagePreview]);

  function selectPreset(presetId: AvatarPresetId) {
    setGuestAvatar(presetId); setGuestImage(null); setGuestImagePreview(null); setError('');
  }

  function selectGuestImage(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; event.target.value = ''; setError('');
    if (!file) return;
    if (!AVATAR_IMAGE_TYPES.includes(file.type)) { setError('Selecciona una imagen JPEG, PNG o WEBP.'); return; }
    if (file.size > MAX_AVATAR_BYTES) { setError('La imagen supera el límite de 5 MB.'); return; }
    setGuestAvatar(undefined); setGuestImage(file); setGuestImagePreview(URL.createObjectURL(file));
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === 'guest') await auth.guest(String(data.get('displayName')), guestImage ?? guestAvatar);
      navigate('/play');
  ...
  return <section className="mx-auto max-w-lg px-4 py-8 sm:px-5 sm:py-12">
    <div className="card">
      <div className="mb-6 text-center"><div className="pokeball-mark mx-auto h-14 w-14 border border-ink" /><h1 className="mt-3 font-display text-3xl font-bold">{mode === 'login' ? 'Qué bueno verte' : mode === 'register' ? 'Crea tu perfil' : 'Crear cuenta temporal'}</h1><p className="mt-1.5 font-semibold text-ink/65">{mode === 'guest' ? 'Tu progreso no se guardará' : 'Tu próxima sala está a pocos segundos.'}</p></div>
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        {mode === 'guest' && <fieldset><legend className="label">Elige un avatar <span className="normal-case tracking-normal text-ink/55">(opcional)</span></legend><div className="grid grid-cols-5 gap-2">{AVATAR_PRESETS.map((preset) => <button key={preset.id} type="button" aria-pressed={guestAvatar === preset.id} aria-label={preset.label} onClick={() => selectPreset(preset.id)} className={`grid min-h-16 place-items-center rounded-2xl border transition ${guestAvatar === preset.id ? 'border-berry bg-berry/10' : 'border-ink/10 hover:border-aqua'}`}><Avatar name={preset.label} avatar={{ type: 'PRESET', value: preset.id }} size="md" /></button>)}<button type="button" aria-pressed={Boolean(guestImage)} aria-label={guestImage ? 'Cambiar mi imagen' : 'Subir mi imagen'} onClick={() => guestImageInput.current?.click()} className={`flex min-h-16 flex-col items-center justify-center gap-1 rounded-2xl border transition ${guestImage ? 'border-berry bg-berry/10' : 'border-dashed border-aqua/45 bg-aqua/5 text-aqua hover:bg-aqua/10'}`}>{guestImagePreview ? <img src={guestImagePreview} alt="" className="h-11 w-11 rounded-full object-cover" /> : <><ImagePlus size={21} aria-hidden="true" /><span className="text-[.68rem] font-extrabold">Tu foto</span></>}</button></div><input ref={guestImageInput} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={selectGuestImage} /><p className="mt-2 text-xs font-bold text-ink/55">JPEG, PNG o WEBP · máximo 5 MB</p></fieldset>}

  ...
  return <section className="mx-auto max-w-lg px-5 py-12">
    <div className="card">
      ...
      <div className="mb-6 grid grid-cols-3 rounded-2xl bg-ink/5 p-1">
        {(['login', 'register', 'guest'] as Mode[]).map((item) => <button key={item} className={`rounded-xl px-2 py-2 text-sm font-extrabold ${mode === item ? 'bg-surface-raised text-ink shadow' : 'text-ink/45'}`} onClick={() => { setMode(item); setError(''); }}>{item === 'login' ? 'Entrar' : item === 'register' ? 'Registro' : 'Invitado'}</button>)}
      </div>

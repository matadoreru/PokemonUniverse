...
export function PlayPage() {
  ...
  return <section className="mx-auto max-w-5xl px-5 py-12">
    <div className="mb-9 text-center"><h1 className="mt-4 font-display text-4xl font-bold">¿Dónde jugamos?</h1></div>
    <div className="grid gap-6 md:grid-cols-2">
      <article className="card flex flex-col items-center text-center"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-electric text-night"><Plus size={32} /></div><h2 className="mt-5 font-display text-2xl font-bold">Crear sala</h2><button className="btn-secondary mt-7 w-full" disabled={busy || !connected} onClick={() => void run(createRoom)}>Crear sala privada</button></article>
      <article className="card flex flex-col items-center text-center"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-aqua text-night"><Copy size={29} /></div><h2 className="mt-5 font-display text-2xl font-bold">Unirse con código</h2><form className="mt-6 w-full" onSubmit={join}><input aria-label="Código de sala" className="field text-center font-display text-2xl uppercase tracking-[.3em]" value={code} onChange={(event) => setCode(event.target.value.toUpperCase().replace(/[^A-Z2-9]/g, '').slice(0, 6))} placeholder="PIKA42" required minLength={6} /><button className="btn-primary mt-3 w-full" disabled={busy || !connected}>Entrar</button></form></article>
    </div>

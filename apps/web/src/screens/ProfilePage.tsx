...
export function ProfilePage() {
  ...
  return <section className="mx-auto max-w-5xl px-5 py-10">
    <div className="card mb-6 flex flex-col items-center gap-5 text-center sm:flex-row sm:text-left"><div className="grid h-24 w-24 place-items-center rounded-[2rem] bg-gradient-to-br from-aqua to-electric font-display text-4xl font-bold">{profile.username[0]?.toUpperCase()}</div><div><span className="label">Perfil de entrenador</span><h1 className="font-display text-4xl font-bold">{profile.username}</h1><p className="font-semibold text-ink/45">Miembro desde {new Date(profile.createdAt).toLocaleDateString('es-ES')}</p></div></div>
    ...
    <article className="card"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-xl bg-electric"><Crosshair /></div><div><span className="label !mb-0">Estadísticas por juego</span><h2 className="font-display text-2xl font-bold">Pokédex Distance</h2></div></div>

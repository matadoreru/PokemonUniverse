...
export function AvatarEditor({ name, avatar, onSaved }: { name: string; avatar: AvatarRef; onSaved(user: AuthUser): void }) {
  const [open, setOpen] = useState(false); const [source, setSource] = useState<string | null>(null); const [zoom, setZoom] = useState(1); const [offsetX, setOffsetX] = useState(0); const [offsetY, setOffsetY] = useState(0); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const imageRef = useRef<HTMLImageElement>(null); const dialogRef = useRef<HTMLDialogElement>(null);
  ...
  return <>
    ...
    <dialog ref={dialogRef} className="modal-dialog" aria-labelledby="avatar-title" onCancel={() => setOpen(false)} onClick={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><div className="p-4 sm:p-6"><div className="flex items-center justify-between"><div><span className="label">Foto de perfil</span><h2 id="avatar-title" className="font-display text-3xl">Elige tu avatar</h2></div><button className="icon-button" onClick={() => setOpen(false)} aria-label="Cerrar"><X /></button></div>
      ...
      {error && <p className="status-error mt-4" role="alert">{error}</p>}<p className="mt-4 text-center text-xs font-bold text-ink/60">JPEG, PNG o WEBP · máximo 5 MB · resultado cuadrado de 256px</p></div></dialog>
  </>;
}

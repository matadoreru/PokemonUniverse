...
export function AppHeader({ user, onLogout }: { user: AuthUser | null; onLogout(): void }) {
  return <header className="mx-auto grid w-full max-w-[96rem] grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center px-3 py-4 sm:px-5 sm:py-5 md:px-8" data-layout="centered-brand">
    ...
    {user ? <div className="flex min-w-0 items-center justify-self-end gap-1 sm:gap-2">
      ...
      <button aria-label="Cerrar sesión" title="Cerrar sesión" className="grid min-h-10 min-w-10 place-items-center rounded-xl transition hover:bg-ink/[0.06]" onClick={onLogout}><LogOut size={20} /></button>
    </div> : <span />}
  </header>;
}

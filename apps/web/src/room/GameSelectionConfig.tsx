      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {availableGames.map((game) => {
          ...
          return <button key={game.id} type="button" disabled={disabled || locked || Boolean(unavailableReason)} aria-pressed={active} title={unavailableReason ?? undefined} onClick={() => toggleGame(game.id)} className={`flex min-h-14 items-center gap-3 rounded-xl border px-3 py-2 text-left transition-colors ${active ? 'border-aqua/60 bg-aqua/10' : 'border-ink/10 bg-surface-raised text-ink/60 hover:border-aqua/40'} disabled:cursor-not-allowed disabled:opacity-70`}>
            <span className="text-xl" aria-hidden="true">{game.icon}</span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-1.5"><strong className="truncate text-sm">{game.name}</strong>{game.experimental && <span className="experimental-badge">Experimental</span>}</span><small className={`font-bold ${unavailableReason ? 'text-berry' : 'text-ink/55'}`}>{unavailableReason ?? `${game.minPlayers}${game.maxPlayers ? `–${game.maxPlayers}` : '+'} jugadores`}</small></span>

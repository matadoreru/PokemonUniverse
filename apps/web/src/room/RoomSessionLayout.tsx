...
function RoomSessionContent() {
  const { connected, error, clearError, room } = useRoom();
  ...
  return <>
    {room && !gameOwnsFeedbackLink && <div className="mx-auto flex w-full max-w-[96rem] justify-end px-3 pt-3 sm:px-5 lg:px-8"><Link className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-extrabold text-ink/60 no-underline transition-colors hover:bg-ink/[.06] hover:text-ink" to={feedbackUrl}><MessageSquareWarning size={18} />Reportar problema o sugerencia</Link></div>}
    <Outlet />

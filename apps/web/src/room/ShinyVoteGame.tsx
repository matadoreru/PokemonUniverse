...
function PlayerPill({ member, result }: { member: RoomMemberView; result?: 'correct' | 'wrong' | undefined }) {
  return <span className={`inline-flex max-w-full items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-extrabold ${result === 'correct' ? 'border-leaf/40 bg-leaf/15 text-leaf' : result === 'wrong' ? 'border-berry/30 bg-berry/10 text-berry' : 'border-ink/10 bg-white/80'}`}>
    <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-gradient-to-br from-aqua/40 to-electric/60 text-[10px] text-ink">{member.displayName.slice(0, 1).toUpperCase()}</span>
...
function OptionCard({ option, voters, selected, disabled, reveal, correct, onSelect }: {
  ...
  const tone = reveal
    ? correct ? 'border-leaf bg-leaf/10 shadow-[0_0_0_5px_rgba(6,214,160,.16)]' : 'border-ink/5 bg-white/60 opacity-55 grayscale'
    : selected ? 'border-berry bg-berry/5 shadow-[0_0_0_4px_rgba(239,71,111,.12)]' : 'border-ink/10 bg-white hover:border-aqua';
  return <button type="button" disabled={disabled} aria-pressed={selected} onClick={onSelect} className={`relative flex min-h-[300px] flex-col overflow-hidden rounded-[1.75rem] border-2 p-3 text-left transition ${tone} ${disabled ? 'cursor-default' : 'hover:-translate-y-1'}`}>
    <div className="flex w-full items-center justify-between"><span className={`grid h-10 w-10 place-items-center rounded-xl border-2 border-ink font-display text-xl font-bold ${correct && reveal ? 'bg-leaf text-white' : 'bg-electric'}`}>{option.id}</span>{selected && !reveal && <span className="chip bg-berry/15 text-berry"><Check size={15} /> Tu elección</span>}{correct && reveal && <span className="chip bg-leaf/20 text-leaf">✨ Shiny correcto</span>}</div>
...
export function ShinyVoteGame({ room, selfId, onAction }: { room: RoomView; selfId: string; onAction(action: unknown): Promise<void> }) {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-5 md:px-8">
    ...
    {active && <div className="mb-5"><div className="mb-1.5 flex items-center justify-between text-sm font-extrabold"><span className="flex items-center gap-1.5"><Clock3 size={17} /> Votación pública</span><span className={remaining <= 5 ? 'timer-pulse text-xl' : ''}>{remaining}s</span></div><div className="h-3 overflow-hidden rounded-full border-2 border-ink bg-white"><div className={`h-full transition-[width] duration-100 ${remaining <= 5 ? 'bg-berry' : 'bg-aqua'}`} style={{ width: `${progress}%` }} /></div></div>}
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_290px]">
      <div>
        ...
        {canVote && <div className="sticky bottom-3 z-10 mx-auto mt-4 flex max-w-lg flex-col items-center gap-2 rounded-2xl border-2 border-ink/10 bg-white/95 p-3 shadow-card backdrop-blur sm:flex-row"><p className="flex-1 text-center font-bold sm:text-left">{draft ? <>Has elegido <strong className="text-berry">{draft}</strong>. Al confirmar no podrás cambiar.</> : 'Selecciona una tarjeta para preparar tu voto.'}</p><button className="btn-primary whitespace-nowrap" disabled={!draft || submitting} onClick={() => void confirmVote()}>{submitting ? 'Confirmando…' : `Confirmar voto${draft ? ` ${draft}` : ''}`}</button></div>}

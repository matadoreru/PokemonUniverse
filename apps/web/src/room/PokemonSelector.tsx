...
export function PokemonSelector({ pokemon, locked, disabled, onSelect }: { pokemon: Pokemon[]; locked: Set<string>; disabled: boolean; onSelect(id: string): Promise<void> }) {
  const [query, setQuery] = useState(''); const deferred = useDeferredValue(query); const [pending, setPending] = useState(''); const [error, setError] = useState('');
  const results = useMemo(() => {
    const needle = normalize(deferred); if (!needle) return pokemon.slice(0, 24);

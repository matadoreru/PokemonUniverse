...
const RaceHeader = memo(function RaceHeader({ game, room, selfId, serverOffset }: { game: PokeddleRacePublicState; room: RoomView; selfId: string; serverOffset: number }) {
  ...
  return <header className="mb-4 rounded-3xl border border-ink/10 bg-surface/90 px-4 py-3.5 shadow-card backdrop-blur sm:px-5">
    <div className="mb-2.5 flex items-center justify-between gap-3 border-b border-ink/[0.07] pb-2.5">
      <div><span className="block text-[10px] font-black uppercase tracking-[.16em] text-ink/40">Carrera multijugador</span><h1 className="font-display text-xl font-bold sm:text-2xl">Pokédle Race</h1></div>
    ...
    <div className="grid grid-cols-3 items-center divide-x divide-ink/10">
      ...
      <div className="text-center"><span className="mb-0.5 block text-[10px] font-black uppercase tracking-[.14em] text-ink/40">Ronda</span><strong className="font-display text-xl tabular-nums sm:text-2xl">{game.roundNumber}<span className="text-ink/30">/{game.maxRounds}</span></strong></div>
      <div className="text-center"><span className="mb-0.5 block text-[10px] font-black uppercase tracking-[.14em] text-ink/40">En carrera</span><strong className="font-display text-xl tabular-nums sm:text-2xl">{game.activePlayerIds.length}</strong></div>
    </div>
...
function SubmittedGuess({ pokemon }: { pokemon: Pokemon | null }) {
  return <div className="flex min-h-20 items-center justify-center gap-3 rounded-2xl bg-leaf/[0.06] px-3 py-3 text-center" role="status">
    ...
    <div className="text-left"><strong className="block font-display text-lg text-leaf">{pokemon ? `${pokemon.name} enviado` : 'Intento enviado'}</strong><span className="text-sm font-bold text-ink/45">Esperando resultados de la ronda…</span></div>
  </div>;
}
...
const RivalCard = memo(function RivalCard({ member, board, currentRound, answered, selected, onSelect }: { member: RoomMemberView; board: PokeddlePublicBoard; currentRound: number; answered: boolean; selected: boolean; onSelect(id: string): void }) {
  ...
  return <button id={`rival-tab-${member.id}`} role="tab" aria-selected={selected} aria-controls="selected-rival-board" onClick={() => onSelect(member.id)} className={`group flex min-h-16 min-w-[12rem] items-center gap-2.5 rounded-2xl border px-3 py-2 text-left transition focus-visible:z-10 ${selected ? 'border-aqua/55 bg-aqua/[0.08] shadow-[inset_3px_0_0_rgba(82,199,232,.75)]' : 'border-ink/10 bg-surface-raised/70 hover:border-ink/25 hover:bg-ink/[0.05]'}`}>
    <Avatar name={member.displayName} avatar={member.avatar} presence={member.presence} size="sm" />
    <span className="min-w-0 flex-1"><strong className="block truncate font-display text-sm">{member.displayName}</strong><small className={`block truncate font-extrabold ${board.solved ? 'text-leaf' : answered ? 'text-aqua' : 'text-ink/45'}`}>{board.solved && '✓ '}{summary.status}</small><small className="block truncate text-[10px] font-bold text-ink/35">{summary.detail}</small></span>
...
export function PokeddleRaceGame({ room, selfId, onAction }: { room: RoomView; selfId: string; onAction(action: unknown): Promise<void> }) {
  ...
  const generationKey = config.generations.join(',');
  ...
  useEffect(() => {
    let active = true;
    setPoolError('');
    void api<{ pokemon: Pokemon[] }>(`/pokemon?generations=${generationKey}`).then((body) => { if (active) setPokemon(body.pokemon); }).catch((caught) => { if (active) setPoolError(caught instanceof Error ? caught.message : 'No se pudo cargar el buscador.'); });
    return () => { active = false; };
  }, [generationKey]);
  ...
  return <section className="mx-auto max-w-[96rem] overflow-x-clip px-3 py-3 sm:px-5 md:py-5">
    ...
    {rivals.length > 0 && <section aria-labelledby="rivals-title">
      ...
      {rival && <div id="selected-rival-board" role="tabpanel" aria-labelledby={`rival-tab-${rival.playerId}`} className="rounded-3xl border border-ink/10 bg-surface/65 p-2.5 sm:p-4"><div className="mb-2.5 flex items-center justify-between gap-3 px-1"><div className="flex min-w-0 items-center gap-2.5">{members.get(rival.playerId) && <Avatar name={members.get(rival.playerId)!.displayName} avatar={members.get(rival.playerId)!.avatar} presence={members.get(rival.playerId)!.presence} size="sm" />}<div className="min-w-0"><span className="block text-[10px] font-black uppercase tracking-[.14em] text-ink/40">Tablero de</span><strong className="block truncate font-display text-lg">{members.get(rival.playerId)?.displayName ?? rival.playerId}</strong></div></div>{rival.solved ? <span className="inline-flex items-center gap-1 text-sm font-extrabold text-leaf"><CheckCircle2 size={17} /> Resuelto</span> : game.answeredPlayerIds.includes(rival.playerId) ? <span className="text-xs font-extrabold text-aqua">Intento enviado ✓</span> : <span className="text-xs font-bold text-ink/40">En carrera</span>}</div><PokeddleBoard board={rival} clues={game.enabledClues} mode="spectator" showLegend={false} /></div>}
    </section>}
  </section>;
}

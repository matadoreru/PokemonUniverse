...
function publicGame(phase: 'ROUND_ACTIVE' | 'VOTING' | 'REVOTE' | 'ROUND_RESULTS' | 'GAME_RESULTS' = 'ROUND_ACTIVE'): PokemonRedFlagPublicState {
  ...
  return {
    gameId: 'pokemon-red-flag', phase, roundNumber: 1, totalRounds: 5, pokemon: { id: 'gengar', name: 'Gengar', sprite: '/gengar.png' }, playerIds: ['p1', 'p2', 'p3'],
...
describe('Pokémon Red Flag client', () => {
  it('renders the Pokémon and private locked writing flow', () => {
    const active = renderToStaticMarkup(createElement(PokemonRedFlagGame, { room: room(publicGame(), { role: 'PLAYER', canSubmit: true, ownAnswer: null, canVote: false, ownVoteAnswerId: null, ownAnswerId: 'rf-1-1' }), selfId: 'p1', onAction: async () => undefined }));
    ...
    const locked = renderToStaticMarkup(createElement(PokemonRedFlagGame, { room: room(publicGame(), { role: 'PLAYER', canSubmit: false, ownAnswer: { id: 'rf-1-1', text: 'Dice que su ex era un Ditto.' }, canVote: false, ownVoteAnswerId: null, ownAnswerId: 'rf-1-1' }), selfId: 'p1', onAction: async () => undefined }));
    expect(locked).toContain('Red flag bloqueada'); expect(locked).toContain('Solo tú puedes verla');
  });

  it('renders anonymous voting, identifies only the own card and supports revote', () => {
    const player: PokemonRedFlagPlayerState = { role: 'PLAYER', canSubmit: false, ownAnswer: { id: 'rf-1-1', text: 'Dice que su ex era un Ditto.' }, canVote: true, ownVoteAnswerId: null, ownAnswerId: 'rf-1-1' };
  ...
  it('reveals authors and renders final statistics after the vote', () => {
    const player: PokemonRedFlagPlayerState = { role: 'PLAYER', canSubmit: false, ownAnswer: { id: 'rf-1-1', text: 'Dice que su ex era un Ditto.' }, canVote: false, ownVoteAnswerId: 'rf-1-2', ownAnswerId: 'rf-1-1' };
    ...
    expect(results).toContain('Las peores señales'); expect(results).toContain('2 votos recibidos'); expect(results).toContain('Continuar sesión');
  });
});

...
import { CheckCircle2, Clock3, Eye, History, Lightbulb, Search, XCircle } from 'lucide-react';
...
function SilhouetteStage({ source }: { source: string }) {
  return <div className="who-silhouette-stage relative mx-auto grid aspect-square w-full max-w-[23rem] place-items-center overflow-hidden rounded-[2.5rem] border border-aqua/20" aria-label="Silueta misteriosa normalizada">
...
function GuessPanel({ pokemon, player, game, serverOffset, participating, loadError, onAction }: { pokemon: Pokemon[]; player: WhosThatPokemonPlayerState; game: WhosThatPokemonPublicState; serverOffset: number; participating: boolean; loadError: string; onAction(action: unknown): Promise<void> }) {
  ...
  if (!participating) return <div className="grid min-h-28 place-items-center rounded-2xl bg-ink/[0.04] text-center"><div><Eye className="mx-auto text-aqua" /><strong className="mt-1 block font-display text-xl">Estás observando</strong><span className="text-sm font-bold text-ink/45">Los espectadores no pueden enviar respuestas.</span></div></div>;
  if (player.solved) return <div className="reveal-pop grid min-h-32 place-items-center rounded-2xl border border-leaf/30 bg-leaf/[0.08] text-center"><div><CheckCircle2 className="mx-auto text-leaf" size={42} /><strong className="mt-1 block font-display text-2xl text-leaf">¡Has acertado!</strong><span className="font-bold text-ink/50">+{player.roundPoints} puntos · esperando al resto…</span></div></div>;
  if (!player.canGuess) return <div className="grid min-h-28 place-items-center rounded-2xl bg-ink/[0.04] font-bold text-ink/45">Esperando al servidor…</div>;
  return <section aria-label="Enviar intento"><div className="mb-2 flex flex-wrap items-end justify-between gap-2"><div><span className="block text-[10px] font-black uppercase tracking-[.15em] text-aqua">Tu respuesta</span><h2 className="flex items-center gap-2 font-display text-xl"><Search size={19} /> Buscar Pokémon</h2></div><span className="text-xs font-bold text-ink/40">{player.attemptCount} intento{player.attemptCount === 1 ? '' : 's'}</span></div>
...
function RoundReveal({ room, game, serverOffset }: { room: RoomView; game: WhosThatPokemonPublicState; serverOffset: number }) {
  ...
  return <section className="mx-auto max-w-6xl px-3 py-5 sm:px-5"><div className="overflow-hidden rounded-[2rem] border border-leaf/25 bg-surface/95 shadow-card reveal-pop"><header className="flex items-start justify-between gap-3 border-b border-ink/10 px-4 py-4 sm:px-6"><div><span className="label !mb-0">Resultado · Ronda {game.roundNumber}</span><h1 className="font-display text-3xl text-leaf sm:text-4xl">¡Era {result.pokemon.name}!</h1></div><ServerTimer deadline={game.nextTransitionAt} serverOffset={serverOffset} label="Siguiente ronda" /></header><div className="grid gap-5 p-4 sm:p-6 lg:grid-cols-[20rem_1fr]"><div className="who-reveal-stage grid min-h-72 place-items-center rounded-3xl bg-leaf/[0.08] p-4"><img src={result.pokemon.sprite} alt={result.pokemon.name} className="h-64 w-64 object-contain [image-rendering:pixelated]" /><strong className="font-display text-2xl">{result.pokemon.name}</strong></div><div className="grid content-start gap-2 sm:grid-cols-2">{room.members.map((member) => { const solve = result.solves[member.id]; const attempts = result.attemptCounts[member.id] ?? 0; return <article key={member.id} className={`flex items-center gap-3 rounded-2xl border p-3 ${solve ? 'border-leaf/30 bg-leaf/[0.07]' : 'border-berry/20 bg-berry/[0.05]'}`}><Avatar name={member.displayName} avatar={member.avatar} size="md" /><div className="min-w-0 flex-1"><strong className="block truncate">{member.displayName}</strong>{member.role === 'SPECTATOR' ? <small className="font-bold text-ink/35">Espectador</small> : solve ? <small className="font-bold text-leaf">✓ {attempts} intento{attempts === 1 ? '' : 's'} · {(solve.elapsedMs / 1_000).toFixed(1)}s</small> : <small className="font-bold text-berry">✗ No acertó · {attempts} intentos</small>}</div><strong className={solve ? 'text-leaf' : 'text-ink/30'}>+{solve?.points ?? 0}</strong></article>; })}</div></div></div></section>;
}
...
export function WhosThatPokemonGame({ room, selfId, onAction }: { room: RoomView; selfId: string; onAction(action: unknown): Promise<void> }) {
  ...
  const generationKey = config.generations.join(','); const [pokemon, setPokemon] = useState<Pokemon[]>([]); const [loadError, setLoadError] = useState('');
  ...
  useEffect(() => { let active = true; setLoadError(''); void api<{ pokemon: Pokemon[] }>(`/pokemon?generations=${generationKey}&includeForms=${config.includeRegionalForms}`).then((body) => { if (active) setPokemon(body.pokemon); }).catch((caught) => { if (active) setLoadError(caught instanceof Error ? caught.message : 'No se pudo cargar el buscador.'); }); return () => { active = false; }; }, [config.includeRegionalForms, generationKey]);
  if (game.phase === 'ROUND_RESULTS' && game.lastRound) return <RoundReveal room={room} game={game} serverOffset={serverOffset} />;
  return <section className="mx-auto max-w-[90rem] overflow-x-clip px-3 py-4 sm:px-5"><header className="mb-4 flex items-center justify-between gap-3 rounded-3xl border border-ink/10 bg-surface/85 px-4 py-3 shadow-card"><div><span className="block text-[10px] font-black uppercase tracking-[.16em] text-ink/40">Ronda {game.roundNumber} / {game.totalRounds}</span><h1 className="font-display text-xl sm:text-3xl">¿Quién es ese Pokémon?</h1></div><ServerTimer deadline={game.roundEndsAt} serverOffset={serverOffset} /></header>
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]"><main className="overflow-hidden rounded-[2rem] border border-ink/10 bg-surface/90 shadow-card"><div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[minmax(18rem,25rem)_1fr] lg:items-center"><div><SilhouetteStage source={game.silhouetteSprite ?? ''} /></div><div className="space-y-4"><div><span className="label !mb-0">La silueta es la pista principal</span><h2 className="font-display text-2xl sm:text-3xl">Reconoce su forma</h2><p className="mt-1 font-bold text-ink/45">Todos juegan a la vez. Los fallos son públicos; los aciertos permanecen ocultos hasta el reveal.</p></div><HintPanel game={game} enabled={config.hintsEnabled} serverOffset={serverOffset} /></div></div><div className="border-t border-ink/[0.08] bg-night/10 p-4 sm:p-5"><GuessPanel pokemon={pokemon} player={player} game={game} serverOffset={serverOffset} participating={Boolean(participating)} loadError={loadError} onAction={onAction} /></div></main><aside className="grid gap-4 md:grid-cols-2 xl:sticky xl:top-4 xl:grid-cols-1"><PlayersPanel room={room} game={game} /><AttemptsPanel room={room} game={game} /></aside></div>
  </section>;
}

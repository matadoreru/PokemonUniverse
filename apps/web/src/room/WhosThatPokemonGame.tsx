...
function GuessPanel({ pokemon, player, game, serverOffset, participating, loadError, onAction }: { pokemon: Pokemon[]; player: WhosThatPokemonPlayerState; game: WhosThatPokemonPublicState; serverOffset: number; participating: boolean; loadError: string; onAction(action: unknown): Promise<void> }) {
  ...
  if (player.solved) return <div className="reveal-pop grid min-h-32 place-items-center rounded-2xl border border-leaf/30 bg-leaf/[0.08] text-center"><div><CheckCircle2 className="mx-auto text-leaf" size={42} /><strong className="mt-1 block font-display text-2xl text-leaf">¡Has acertado!</strong><span className="font-bold text-ink/65">{ordinal(player.solveOrder!)} en acertar · +{player.roundPoints} puntos</span><small className="block font-bold text-ink/55">Esperando al resto…</small></div></div>;
...
function PlayersPanel({ room, game }: { room: RoomView; game: WhosThatPokemonPublicState }) {
  ...
  return <section className="rounded-2xl border border-ink/10 bg-surface/80 p-4"><h2 className="mb-3 font-display text-xl">Jugadores</h2><div className="space-y-2">{room.members.map((member) => { const order = solved.get(member.id); const participating = member.role === 'PLAYER'; return <div key={member.id} className={`flex items-center gap-2.5 rounded-xl px-2.5 py-2 ${order ? 'bg-leaf/[0.08]' : 'bg-ink/[0.035]'}`}><Avatar name={member.displayName} avatar={member.avatar} presence={member.presence} size="sm" /><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{member.displayName}</strong><small className="font-bold text-ink/55">{game.scores[member.id] ?? 0} pts</small></div>{!participating ? <span className="flex items-center gap-1 text-xs font-bold text-ink/55"><Eye size={15} /> Espectador</span> : order ? <span className="flex items-center gap-1 text-xs font-black text-leaf"><CheckCircle2 size={16} /> {ordinal(order)} en acertar</span> : <span className="text-xs font-bold text-ink/60">buscando…</span>}</div>; })}</div></section>;
}
...
function RoundReveal({ room, game, serverOffset }: { room: RoomView; game: WhosThatPokemonPublicState; serverOffset: number }) {
  ...
  const ordered = room.members.filter((member) => member.role !== 'SPECTATOR').sort((left, right) => (result.solves[left.id]?.solveOrder ?? Infinity) - (result.solves[right.id]?.solveOrder ?? Infinity));
  return <section className="mx-auto max-w-6xl px-3 py-5 sm:px-5"><div className="overflow-hidden rounded-2xl border border-leaf/25 bg-surface/95 shadow-card reveal-pop"><header className="flex items-start justify-between gap-3 border-b border-ink/10 px-4 py-4 sm:px-6"><div><span className="label !mb-0">Resultado · Ronda {game.roundNumber}</span><h1 className="font-display text-3xl text-leaf sm:text-4xl">¡Era {result.pokemon.name}!</h1><p className="text-sm font-bold text-ink/60">{whoPokemonScoringLabel()}</p></div><ServerTimer deadline={game.nextTransitionAt} serverOffset={serverOffset} label="Siguiente ronda" /></header><div className="grid gap-5 p-4 sm:p-6 lg:grid-cols-[20rem_1fr]"><div className="who-reveal-stage grid min-h-72 place-items-center rounded-2xl bg-leaf/[0.08] p-4"><img src={result.pokemon.sprite} alt={result.pokemon.name} className="h-64 w-64 object-contain [image-rendering:pixelated]" /><strong className="font-display text-2xl">{result.pokemon.name}</strong></div><div className="grid content-start gap-2 sm:grid-cols-2">{ordered.map((member) => { const solve = result.solves[member.id]; const attempts = result.attemptCounts[member.id] ?? 0; return <article key={member.id} className={`flex items-center gap-3 rounded-2xl border p-3 ${solve ? 'border-leaf/30 bg-leaf/[0.07]' : 'border-berry/20 bg-berry/[0.05]'}`}><strong className="w-8 text-center text-xl">{solve ? solve.solveOrder <= 3 ? ['🥇', '🥈', '🥉'][solve.solveOrder - 1] : solve.solveOrder : '—'}</strong><Avatar name={member.displayName} avatar={member.avatar} size="md" /><div className="min-w-0 flex-1"><strong className="block truncate">{member.displayName}</strong>{solve ? <><small className="block font-bold text-leaf">{ordinal(solve.solveOrder)} · {attempts} intento{attempts === 1 ? '' : 's'} · {(solve.elapsedMs / 1_000).toFixed(1)}s</small><small className="font-bold text-ink/55">Rapidez {solve.speedPoints} + orden {solve.placementBonus}</small></> : <small className="font-bold text-berry">✗ No acertó · {attempts} intentos</small>}</div><strong className={solve ? 'text-leaf' : 'text-ink/50'}>+{solve?.points ?? 0}</strong></article>; })}</div></div></div></section>;
}

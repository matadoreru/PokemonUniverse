    ...
    <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="space-y-6">
        <article className="card">
          <div className="mb-4 flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-aqua"><Settings2 /></div><div><span className="label !mb-0">Seleccionar minijuego</span><h2 className="font-display text-2xl font-bold">Pokémon Minigame Hub</h2></div></div>
          <div className="mb-7 grid gap-3 sm:grid-cols-2">{room.availableGames.map((game) => {
            const selected = game.id === room.selectedGameId;
            return <button key={game.id} type="button" disabled={!host} aria-pressed={selected} onClick={() => selectGame(game.id)} className={`min-h-36 rounded-2xl border-2 p-4 text-left transition ${selected ? 'border-ink bg-electric shadow-[0_0_0_3px_rgba(255,209,102,.2)]' : 'border-ink/10 bg-white hover:border-aqua'} ${!host ? 'cursor-default' : ''}`}>
              ...
              <span className="mt-1 block text-sm font-bold text-ink/50">{game.description}</span>
              {selected && <span className="mt-3 inline-flex rounded-full bg-ink px-2 py-1 text-xs font-extrabold text-white">Seleccionado</span>}
            </button>;
          })}</div>

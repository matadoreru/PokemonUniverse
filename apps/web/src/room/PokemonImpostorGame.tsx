...
function RoleCard({ player, game }: { player: PokemonImpostorPlayerState; game: PokemonImpostorPublicState }) {
  if (!player.role) return <div className="card text-center"><Eye className="mx-auto text-aqua" size={42} /><h2 className="mt-3 font-display text-2xl">Espectador</h2><p className="font-bold text-ink/65">Observas la partida en directo.</p></div>;
...
export function PokemonImpostorGame({ room, selfId, onAction }: { room: RoomView; selfId: string; onAction(action: unknown): Promise<void> }) {
  ...
  const remaining = Math.ceil(useRemainingMs(deadline, serverOffset) / 1_000);
  ...
  const { pokemon } = usePokemonPool({ generations, enabled: player.canGuessPokemon });
  const orderedPlayers = game.phase === 'CLUE_PHASE'
    ? [...game.clueOrder, ...game.playerIds.filter((id) => !game.clueOrder.includes(id))]
    : game.playerIds;
  ...
  async function guessPokemon(pokemonId: string) {
    if (!player.canGuessPokemon || submitting) return;
    setSubmitting(true); setError('');
    try { await onAction({ type: 'GUESS_POKEMON', pokemonId }); setShowGuess(false); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'No se pudo registrar el intento.'); }
    finally { setSubmitting(false); }
  }

  const phaseTitle = game.phase === 'ROLE_REVEAL' ? 'Descubre tu rol'
    : game.phase === 'CLUE_PHASE' ? `Turno de ${game.currentCluePlayerId ? memberName(members, game.currentCluePlayerId) : 'pistas'}`
  ...
  return <section className="mx-auto max-w-7xl px-4 py-5 md:px-8">
    ...
    {game.phase === 'ROLE_REVEAL' && <div className="mb-5 rounded-2xl border border-electric/40 bg-electric/10 p-3 text-center font-bold">Esta información es privada. La fase de pistas comenzará automáticamente.</div>}
    {game.phase === 'VOTE_RESULTS' && game.lastVoteResult && <div className={`mb-5 rounded-2xl border p-4 text-center ${game.lastVoteResult.kind === 'TIE' ? 'border-electric bg-electric/15' : 'border-aqua bg-aqua/15'}`}><strong className="font-display text-2xl">{game.lastVoteResult.kind === 'TIE' ? '⚔️ Empate: habrá una nueva votación' : `${memberName(members, game.lastVoteResult.eliminatedId!)} recibió más votos`}</strong><div className="mt-3 flex flex-wrap justify-center gap-2">{Object.entries(game.lastVoteResult.tallies).filter(([, count]) => count > 0).map(([id, count]) => <span key={id} className="chip">{memberName(members, id)} · {count} {count === 1 ? 'voto' : 'votos'}</span>)}</div><div className="mx-auto mt-3 grid max-w-2xl gap-2 sm:grid-cols-2">{game.aliveIds.map((voterId) => { const vote = game.lastVoteResult!.votes[voterId]; return <div key={voterId} className="rounded-xl bg-surface/70 px-3 py-2 text-sm font-bold">{memberName(members, voterId)} → {vote ? memberName(members, vote.targetId) : 'Sin voto'}</div>; })}</div></div>}
    {game.phase === 'ELIMINATION' && game.eliminationReveal && <div className="reveal-pop mb-5 rounded-2xl border border-berry bg-berry/10 p-5 text-center"><Skull className="mx-auto text-berry" size={44} /><strong className="mt-2 block font-display text-3xl">{memberName(members, game.eliminationReveal.playerId)}</strong><p className="font-display text-xl">era {game.eliminationReveal.role === 'IMPOSTOR' ? 'IMPOSTOR' : 'INOCENTE'}</p>{game.winnerTeam && <p className="mt-2 font-extrabold text-berry">Partida terminada: ganan {game.winnerTeam === 'IMPOSTORS' ? 'los impostores' : 'los inocentes'}.</p>}</div>}
    <div className="grid gap-5 lg:grid-cols-[280px_1fr]"><aside><RoleCard player={player} game={game} /></aside><div className="space-y-5">
      <div className="card !p-4"><div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 font-display text-xl"><Users size={20} className="text-aqua" /> Orden y pistas</h2><span className="chip">{game.aliveIds.length} vivos</span></div><div className="grid gap-3 md:grid-cols-2">{orderedPlayers.map((id) => { const currentTurn = game.phase === 'CLUE_PHASE' && game.currentCluePlayerId === id; const orderIndex = game.clueOrder.indexOf(id); return <article key={id} className={`rounded-2xl border p-3 ${game.eliminatedIds.includes(id) ? 'border-ink/5 bg-ink/[.03] opacity-65' : currentTurn ? 'border-aqua bg-aqua/10 shadow-[0_0_0_4px_rgba(76,195,223,.10)]' : 'border-ink/10 bg-surface-raised'}`}><div className="flex items-center justify-between gap-2"><strong className="truncate font-display text-lg">{orderIndex >= 0 && <span className="mr-2 text-ink/55">{orderIndex + 1}.</span>}{memberName(members, id)}</strong>{game.eliminatedIds.includes(id) ? <span className="chip text-xs">Eliminado</span> : currentTurn ? <span className="chip bg-aqua/15 text-aqua">▶ Su turno · {remaining}s</span> : game.phase === 'VOTING' ? <span className={`text-sm font-extrabold ${game.voteCompletedIds.includes(id) ? 'text-leaf' : 'text-ink/55'}`}>{game.voteCompletedIds.includes(id) ? '✓ Votó' : '…'}</span> : null}</div><div className="mt-2 space-y-1.5">{Array.from({ length: game.roundNumber }, (_, index) => index + 1).map((round) => { const entry = game.clues[round]?.[id]; const isCurrent = round === game.roundNumber && currentTurn; const waiting = round === game.roundNumber && game.phase === 'CLUE_PHASE' && orderIndex > game.currentClueTurnIndex; return <p key={round} className="rounded-lg bg-ink/[.035] px-2 py-1.5 text-sm"><span className="mr-2 font-extrabold text-ink/60">R{round}</span>{entry?.status === 'SUBMITTED' ? `“${entry.text}”` : entry ? <span className="text-berry">Sin pista · {entry.status === 'TIMEOUT' ? 'tiempo agotado' : 'desconectado'}</span> : isCurrent ? <span className="font-extrabold text-aqua">TU TURNO</span> : waiting ? <span className="text-ink/55">Esperando…</span> : <span className="text-ink/55">Sin pista</span>}</p>; })}</div></article>; })}</div></div>
      {game.phase === 'CLUE_PHASE' && player.canSubmitClue && <div className="card !p-4"><label className="label flex items-center gap-2"><MessageSquareText size={17} /> Tu pista</label><div className="flex flex-col gap-3 sm:flex-row"><input className="field flex-1" maxLength={50} value={clueText} onChange={(event) => setClueText(event.target.value)} placeholder="Escribe algo relacionado…" onKeyDown={(event) => { if (event.key === 'Enter') void submitClue(); }} /><button className="btn-primary justify-center" disabled={!clueText.trim() || clueLength > 25 || submitting} onClick={() => void submitClue()}><Send size={18} /> Enviar pista</button></div><p className={`mt-2 text-right text-sm font-extrabold ${clueLength > 25 ? 'text-berry' : 'text-ink/60'}`}>{clueLength}/25 caracteres</p></div>}
      {game.phase === 'CLUE_PHASE' && player.ownClue?.status === 'SUBMITTED' && <div className="rounded-2xl border border-leaf/40 bg-leaf/10 p-3 text-center font-extrabold text-leaf"><Check className="mr-2 inline" />Tu pista está enviada y bloqueada.</div>}
      {game.phase === 'CLUE_PHASE' && player.role === 'IMPOSTOR' && <div className="card !p-4"><h2 className="flex items-center gap-2 font-display text-xl"><Brain className="text-berry" /> Adivinar el Pokémon</h2>{player.canGuessPokemon ? <><p className="mt-1 font-bold text-ink/65">Tienes un único intento durante la partida, aunque no sea tu turno de pista.</p><button className="btn-secondary mt-3" onClick={() => setShowGuess((value) => !value)}>{showGuess ? 'Cerrar buscador' : 'Adivinar Pokémon'}</button>{showGuess && <div className="mt-3"><PokemonSelector pokemon={pokemon} locked={new Set()} disabled={submitting} variant="embedded" onSelect={guessPokemon} /></div>}</> : <p className={`mt-2 font-extrabold ${player.guessCorrect ? 'text-leaf' : 'text-berry'}`}>{player.guessCorrect ? '¡Adivinaste el Pokémon!' : 'Ya utilizaste tu intento.'}</p>}</div>}
      {game.phase === 'VOTING' && <div className="card !p-4"><h2 className="mb-3 flex items-center gap-2 font-display text-xl"><Vote className="text-berry" /> {game.votingRound > 1 ? 'Vota entre los jugadores empatados' : 'Elige a quien quieres expulsar'}</h2><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{game.voteCandidateIds.map((id) => { const self = id === selfId; return <button type="button" key={id} disabled={!player.canVote || self || submitting} aria-pressed={voteDraft === id} onClick={() => setVoteDraft(id)} className={`rounded-2xl border p-3 text-left font-display text-lg transition ${voteDraft === id ? 'border-berry bg-berry text-white' : 'border-ink/10 bg-surface-raised'} ${self ? 'cursor-not-allowed opacity-35' : ''}`}>{memberName(members, id)}{self && <span className="ml-2 text-xs">(tú)</span>}</button>; })}</div>{player.canVote ? <button className="btn-primary mx-auto mt-4" disabled={!voteDraft || submitting} onClick={() => void submitVote()}><Vote size={18} /> Confirmar voto</button> : player.ownVote ? <p className="mt-4 text-center font-extrabold text-leaf"><Check className="mr-2 inline" />Tu voto está bloqueado. El objetivo permanece secreto.</p> : null}</div>}

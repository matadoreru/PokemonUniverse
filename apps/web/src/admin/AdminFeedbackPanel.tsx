...
export function AdminFeedbackPanel() {
  ...
  }, [page, reloadVersion, search, status, type]);
  ...
  return <section className="space-y-5" aria-labelledby="feedback-admin-title">
    <div><h2 id="feedback-admin-title" className="font-display text-2xl font-bold">Feedback de jugadores</h2><p className="mt-1 max-w-3xl text-sm font-semibold text-ink/65">Revisa problemas y sugerencias enviados desde los minijuegos.</p></div>
    ...
    <div className="panel overflow-hidden">
      <div className="grid gap-3 border-b border-ink/10 p-3 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
        <label className="relative"><span className="sr-only">Buscar feedback</span><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink/45" size={18} /><input className="field pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar texto, jugador, sala o juego" /></label>

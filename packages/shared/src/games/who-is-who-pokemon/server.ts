...
function finish(state: WhoIsWhoState, winnerTeam: WhoIsWhoTeam | null): WhoIsWhoState {
  ...
  return { ...state, phase: 'GAME_RESULTS', roundEndsAt: null, winnerTeam, scores, playerStats };
}
...
export const whoIsWhoPokemonGame: MiniGameModule<WhoIsWhoPokemonConfig, WhoIsWhoState, WhoIsWhoPokemonAction, WhoIsWhoPublicState> = {
  ...
  getCursorChannel(state, playerId) {
    const team = teamFor(state, playerId);
    return state.phase === 'TURN_ACTIVE' && team ? { playerIds: state.teams[team].playerIds, targetIds: state.board.map((pokemon) => pokemon.id) } : null;
  },
  ...
  start(state, context) { const ready = teams.every((team) => state.teams[team].secretPokemonId !== null); return { ...state, phase: 'TURN_ACTIVE', roundStartedAt: ready ? context.now : null, roundEndsAt: ready ? context.now + state.config.turnSeconds * 1_000 : state.config.selectionSeconds > 0 ? context.now + state.config.selectionSeconds * 1_000 : null }; },
  handleAction(state, playerId, action, context): GameActionResult<WhoIsWhoState> {
    ...
    if (action.type === 'SELECT_SECRET') {
      if (state.roundEndsAt !== null && context.now >= state.roundEndsAt) return { state, accepted: false, error: 'El tiempo de selección ha terminado.' };
  ...
  handleTimeout(state, context) {
    if (state.phase !== 'TURN_ACTIVE' || state.roundEndsAt === null || context.now < state.roundEndsAt) return state;
    if (teams.some((team) => state.teams[team].secretPokemonId === null)) {
      const nextTeams = { ...state.teams };
      for (const team of teams) {
        if (nextTeams[team].secretPokemonId !== null) continue;
        const secret = choose(state.board, 1, context.random)[0]!;
        nextTeams[team] = { ...nextTeams[team], secretPokemonId: secret.id };
      }
      return { ...state, teams: nextTeams, roundStartedAt: context.now, roundEndsAt: context.now + state.config.turnSeconds * 1_000 };
    }
    return advanceTurn(state, context);
  },
  handlePresenceChange(state, context) { if (state.phase === 'GAME_RESULTS') return state; for (const team of teams) if (state.teams[team].playerIds.every((id) => !context.players.some((player) => player.id === id && player.active !== false))) return finish(state, team === 'BLUE' ? 'RED' : 'BLUE'); return state; },
  // The public projection intentionally does not need the player-specific context.
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  getPublicState(state, context) { const reveal = state.phase === 'GAME_RESULTS'; const revealedSecrets = Object.fromEntries(teams.map((team) => [team, reveal ? state.board.find((pokemon) => pokemon.id === state.teams[team].secretPokemonId) ?? null : null])) as WhoIsWhoPublicState['revealedSecrets']; return { gameId: 'who-is-who-pokemon', phase: state.phase, board: state.board, teams: { BLUE: { playerIds: state.teams.BLUE.playerIds, secretReady: state.teams.BLUE.secretPokemonId !== null }, RED: { playerIds: state.teams.RED.playerIds, secretReady: state.teams.RED.secretPokemonId !== null } }, currentTeam: state.currentTeam, roundNumber: state.roundNumber, turnNumber: state.turnNumber, totalRounds: state.config.rounds, roundStartedAt: state.roundStartedAt, roundEndsAt: state.roundEndsAt, guesses: state.guesses.map((guess) => ({ ...guess, pokemonId: guess.pokemonId })), winnerTeam: state.winnerTeam, revealedSecrets, results: state.phase === 'GAME_RESULTS' ? buildWhoIsWhoResults(state) : null }; },
  ...
  getResults(state) { return buildWhoIsWhoResults(state); },
};

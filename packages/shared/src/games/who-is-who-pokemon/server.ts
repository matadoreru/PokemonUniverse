...
function beginNextRound(state: WhoIsWhoState, context: GameContext): WhoIsWhoState {
  const candidates = state.board.filter((pokemon) => pokemon.id !== state.teams.BLUE.secretPokemonId && pokemon.id !== state.teams.RED.secretPokemonId);
  const source = candidates.length >= 2 ? candidates : state.board;
  const secrets = choose(source, 2, context.random);
  return { ...state, currentTeam: 'BLUE', roundNumber: state.roundNumber + 1, turnNumber: state.turnNumber + 1,
    teams: {
      BLUE: { ...state.teams.BLUE, secretPokemonId: secrets[0]!.id, discardedPokemonIds: [] },
      RED: { ...state.teams.RED, secretPokemonId: secrets[1]!.id, discardedPokemonIds: [] },
...
export const whoIsWhoPokemonGame: MiniGameModule<WhoIsWhoPokemonConfig, WhoIsWhoState, WhoIsWhoPokemonAction, WhoIsWhoPublicState> = {
  ...
  handleTimeout(state, context) { return state.phase === 'TURN_ACTIVE' && context.now >= (state.roundEndsAt ?? 0) ? advanceTurn(state, context) : state; },

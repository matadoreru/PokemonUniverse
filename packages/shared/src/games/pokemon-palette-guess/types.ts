...
export interface PokemonPaletteRoundResult { pokemon: { id: string; name: string; sprite: string; generation: number }; palette: string[]; solves: Record<string, PokemonPaletteSolve>; attemptCounts: Record<string, number> }
...
export interface PokemonPaletteGuessPublicState {
  gameId: 'pokemon-palette-guess'; phase: GamePhase; roundNumber: number; totalRounds: number; colors: string[]; attempts: PokemonPaletteAttempt[]; solvedPlayers: Array<{ playerId: string; solveOrder: number }>;
  scores: Record<string, number>; roundStartedAt: number | null; roundEndsAt: number | null; nextTransitionAt: number | null;
  lastRound: ({ pokemon: { name: string; sprite: string; generation: number }; palette: string[]; solves: Record<string, PokemonPaletteSolve>; attemptCounts: Record<string, number> }) | null; results: GameResults | null;
}
